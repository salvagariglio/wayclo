...
function Item({ i, openIndex, setOpenIndex, q, a }) {
    ...
    // Recalcular altura para animar (y soportar cambios de contenido)
    React.useLayoutEffect(() => {
        ...
        if (isOpen) {
            ...
            setHeight(h);
            // tras la transición, dejamos "auto"
            const onEnd = () => setHeight("auto");
            el.addEventListener("transitionend", onEnd, { once: true });
            return () => el.removeEventListener("transitionend", onEnd);
        } else {
            // si estaba en auto, primero fijamos altura actual, forzamos reflow y luego 0
            if (height === "auto") {
                const h = el.scrollHeight;
                setHeight(h);
                // forzar reflow
 ...
 * - className?: string
 * - ringed?: boolean  (añade borde y divide-y)
 */
